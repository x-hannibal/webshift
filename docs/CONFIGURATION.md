...
| `server.max_result_length` | `WEBSHIFT_MAX_RESULT_LENGTH` | — | `8000` | **Per-page character cap.** No single page can exceed this, even if the budget allows more. This prevents one very long page from eating all the budget. Set to `0` to disable the cap entirely (no truncation). |
