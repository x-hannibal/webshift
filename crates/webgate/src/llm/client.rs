...
//!
//! Port of `../mcp-webgate/src/mcp_webgate/llm/client.py`.
...
impl LlmClient {
    ...
    ///
    /// Returns `WebgateError::Llm` if LLM is not enabled or on API errors.
    pub async fn chat(
        &self,
        ...
        if !self.config.enabled {
            return Err(crate::WebgateError::Llm(
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| crate::WebgateError::Llm(format!("LLM request failed: {e}")))?;

        let status = resp.status();
        if !status.is_success() {
            return Err(crate::WebgateError::Llm(format!("LLM API error HTTP {status}")));
        }

        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| crate::WebgateError::Llm(format!("LLM response parse error: {e}")))?;
