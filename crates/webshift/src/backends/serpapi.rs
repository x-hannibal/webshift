...
impl SerpapiBackend {
    pub fn new(config: &SerpapiConfig) -> Result<Self, crate::WebshiftError> {
        if config.api_key.is_empty() {
            return Err(crate::WebshiftError::Backend(
                "SerpAPI requires WEBSHIFT_SERPAPI_API_KEY to be set".into(),
            ));
        }
        Ok(Self {
            config: config.clone(),
...
impl SearchBackend for SerpapiBackend {
    async fn search(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("serpapi request failed: {e}")))?;
        ...
        let data: serde_json::Value = resp
            ...
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("serpapi parse error: {e}")))?;
...
mod tests {
    ...
    fn serpapi_new_empty_api_key_returns_error() {
        ...
        assert!(result.unwrap_err().to_string().contains("WEBSHIFT_SERPAPI_API_KEY"));
    }
