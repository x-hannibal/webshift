...
//!
//! Requires a Microsoft Azure API key (`WEBSHIFT_BING_API_KEY`).
...
impl BingBackend {
    pub fn new(config: &BingConfig) -> Result<Self, crate::WebshiftError> {
        if config.api_key.is_empty() {
            return Err(crate::WebshiftError::Backend(
                "Bing Web Search requires WEBSHIFT_BING_API_KEY to be set".into(),
            ));
        }
        Ok(Self {
...
impl SearchBackend for BingBackend {
    async fn search(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("bing request failed: {e}")))?;
        ...
        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("bing parse error: {e}")))?;
...
mod tests {
    ...
    fn bing_new_empty_api_key_returns_error() {
        ...
        assert!(result.unwrap_err().to_string().contains("WEBSHIFT_BING_API_KEY"));
    }
