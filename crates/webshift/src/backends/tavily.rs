...
impl TavilyBackend {
    pub fn new(config: &TavilyConfig) -> Result<Self, crate::WebshiftError> {
        if config.api_key.is_empty() {
            return Err(crate::WebshiftError::Backend(
                "Tavily Search requires WEBSHIFT_TAVILY_API_KEY to be set".into(),
            ));
        }
        Ok(Self {
...
impl SearchBackend for TavilyBackend {
    async fn search(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("tavily request failed: {e}")))?;
        ...
        let data: serde_json::Value = resp
            ...
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("tavily parse error: {e}")))?;
...
mod tests {
    ...
    fn tavily_new_empty_api_key_returns_error() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("WEBSHIFT_TAVILY_API_KEY"));
    }
