...
//!
//! Requires a Google API key (`WEBSHIFT_GOOGLE_API_KEY`) and a Custom Search
...
impl GoogleBackend {
    pub fn new(config: &GoogleConfig) -> Result<Self, crate::WebshiftError> {
        if config.api_key.is_empty() {
            return Err(crate::WebshiftError::Backend(
                "Google Custom Search requires WEBSHIFT_GOOGLE_API_KEY to be set".into(),
            ));
        }
        if config.cx.is_empty() {
            return Err(crate::WebshiftError::Backend(
                "Google Custom Search requires WEBSHIFT_GOOGLE_CX (Custom Search Engine ID)".into(),
            ));
        }
        Ok(Self {
...
impl SearchBackend for GoogleBackend {
    async fn search(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("google request failed: {e}")))?;
        ...
        let data: serde_json::Value = resp
            .json()
            .await
            .map_err(|e| crate::WebshiftError::Backend(format!("google parse error: {e}")))?;
...
mod tests {
    ...
    fn google_new_empty_api_key_returns_error() {
        ...
        assert!(result.unwrap_err().to_string().contains("WEBSHIFT_GOOGLE_API_KEY"));
    }
    ...
    fn google_new_empty_cx_returns_error() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("WEBSHIFT_GOOGLE_CX"));
    }
